...
%% Plots
time2 = i_c2.time + 0.5276;
%time3 = i_c2.time + 1.28 + 0.065;

subplot(2,1,1)
hold on;
plot(p_c2.time, p_c2.data./s_N, 'Color', 'b', 'LineWidth', 1.2);
plot(q_c2.time, q_c2.data./s_N, 'Color', 'r', 'LineWidth', 1.2);
plot(time2, double(p_c2_ref.data)./s_N, 'Color', [0 0.6 0], 'LineStyle','--', 'LineWidth', 1.2);
%plot(time2, double(q_c2_ref.data)./s_N, 'Color', [0 0.8 0.8], 'LineStyle','--', 'LineWidth', 1.2);
grid on;
ylabel('Power (p.u.)');
legend('$p_c$', '$q_c$', '$p_{c,ref}$', 'Location', 'southeast', 'Interpreter', 'latex', 'FontSize', 14)
hold off;
h = gca;
h.XAxis.Visible = 'off';
set(gca,'XTick',(16.8:0.05:16.8+0.1));
...
% plot(p_c2.time, p_c2.data./s_N, 'Color', 'b', 'LineWidth', 1.2);
% plot(q_c2.time, q_c2.data./s_N, 'Color', 'r', 'LineWidth', 1.2);
% plot(time3, double(p_c2_ref.data)./s_N, 'Color', [0 0.6 0], 'LineStyle','--', 'LineWidth', 1.2);
% plot(time3, double(q_c2_ref.data)./s_N, 'Color', [0 0.8 0.8], 'LineStyle','--', 'LineWidth', 1.2);
% grid on;
% ylabel('Power (p.u.)');
% legend('$p_c$', '$q_c$', '$p_{c,ref}$', '$q_{c,ref}$', 'Location', 'southeast', 'Interpreter', 'latex', 'FontSize', 11)
% hold off;
% h = gca;
% h.YAxis.Visible = 'off';
% h.XAxis.Visible = 'off';
% set(gca,'XTick',(29.05:0.05:29.05+0.1));
% set(gca,'YTick',(-0.5:0.5:0.5));
% xlim([29.05 29.05+0.1])
% ylim([-0.75 0.75])

subplot(2,1,2)
hold on;
plot(u_pcc.time, u_pcc_a./u_N, 'Color', 'r', 'LineWidth', 1.2)
plot(u_pcc.time, u_pcc_b./u_N, 'Color', [0 0.6 0], 'LineWidth', 1.2)
plot(u_pcc.time, u_pcc_c./u_N, 'Color', 'b', 'LineWidth', 1.2)
grid on;
ylabel('Voltage (p.u.)');
xlabel('Time (s)');
legend('$u_g^a$', '$u_g^b$', '$u_g^c$', 'Location', 'east', 'Interpreter', 'latex', 'FontSize', 14)
hold off;
set(gca,'XTick',(16.8:0.05:16.8+0.1));
xlim([16.8 16.8+0.1])
ylim([-1.2 1.2])
...
% h = gca;
% h.YAxis.Visible = 'off';
% legend('$u_g^a$', '$u_g^b$', '$u_g^c$', 'Location', 'east', 'Interpreter', 'latex')
% hold off;
% xlabel('Time (s)');
% set(gca,'XTick',(29.05:0.05:29.05+0.1));
% xlim([29.05 29.05+0.1])
...
% ylabel('Voltage (p.u.)');
% xlabel('Time (s)');
% legend('$u_g^a$', '$u_g^b$', '$u_g^c$', 'Location', 'east', 'Interpreter', 'latex', 'FontSize', 11)
% hold off;
% xlim([22.532 22.532+0.065])
% ylim([-1.5 1.5])

% int1 = [31.4572 31.4572+0.065];
% start1 = 31.4572;
% end1 = start1 + 0.065;
% ticks1 = start1:0.065:end1;
% 
